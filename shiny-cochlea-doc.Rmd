...
GO_anno <- read.delim("data-files/GOBPannotation.tab", stringsAsFactors = F)
GO_terms <- subset(GO_anno, select = c("Term.Accession", "Term"))
GO_terms_unique <- unique(GO_terms)

write_delim(GO_terms_unique, path = "./app-files/GOids_terms.txt", 
            delim = "\t")
...
write_delim(GO_table, path = "./app-files/GO_terms.txt", delim = "\t")
GO_table2 <- as.data.frame((read.delim("./app-files/GO_terms.txt", stringsAsFactors = F)))
...
test <- getGeneMatrix("GO:0006956", GO_table, norm_expr.copy)
